...
export default function RecentObservations({ observations }: { observations: UserObservation[] }) {
  return (
    <div className='h-full rounded-xl border border-border bg-card flex flex-col'>
      <div className='flex items-center justify-between gap-4 px-4 sm:px-5 pt-4 sm:pt-5 pb-1'>
        <h2 className='font-heading text-lg font-bold text-card-foreground'>Recent observations</h2>
        <Link
          href='/observations'
      ...
      {observations.length === 0 ? (
        <div className='flex-1 flex flex-col items-center justify-center text-center px-6 py-8'>
          ...
          <p className='text-sm text-muted-foreground'>Your recent observations will appear here.</p>
        </div>
      ) : (
